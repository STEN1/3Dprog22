...
void main()
{
    ...
    vec3 lightResult = ambient + diffuse + (specular * vec3(texture(material.specular, uv)));

    if (drawMode == 0)
        FragColor = vec4(lightResult, 1.0) * texture(material.diffuse, uv);
    else if (drawMode == 1)
